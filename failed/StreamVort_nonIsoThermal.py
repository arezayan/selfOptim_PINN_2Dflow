...
def plot_results(PINN_u , PINN_v , PINN_T ,  ff):
    ...
    truth = torch.tensor(df[['u' ,'v' ,'T']].values, dtype=torch.float32 , requires_grad = True)
...
def train_pinn(PINN_psi, PINN_omega , PINN_T , filename_data,
    ...
    #test-train Split collocation_points
    ...
    X_interior_train, X_interior_test, Y_interior_train, Y_interior_test = train_test_split(torch.cat((x_interior , y_interior) ,dim = 1), output_interior, test_size=0.2, random_state=42)
    ...
    X_boundary_train, X_boundary_test, Y_boundary_train, Y_boundary_test = train_test_split(torch.cat((x_boundary , y_boundary) ,dim = 1), output_boundary, test_size=0.2, random_state=42)
    ...
    def objective(trial):
        ...
        for epoch in range(num_epochs_trial):
            ...
            pde_loss = residula_loss(PINN_psi , PINN_omega  , PINN_T , X_c_train , Y_c_train )
            bc_loss = boundary_condition_loss(PINN_psi , PINN_omega  , PINN_T ,X_boundary_train[: , 0].reshape(-1 , 1) , X_boundary_train[: ,1].reshape(-1 , 1) , Y_boundary_train)
            interior_loss = data_loss(PINN_psi , PINN_omega  , PINN_T , X_interior_train[: , 0].reshape(-1 , 1) , X_interior_train[: ,1].reshape(-1 , 1) , Y_interior_train)
    ...
    PINN_psi.apply(init_xavier)
    PINN_omega.apply(init_xavier)
    ...
    optimizer_adam_T = optim.Adam(PINN_T.parameters(), lr=1e-3)
    optimizer_adam_psi = optim.Adam(PINN_psi.parameters(), lr=1e-3)
    optimizer_adam_omega = optim.Adam(PINN_omega.parameters(), lr=1e-3)
    
    #stage3-1: Adam Reduce LR on PLateaue
    scheduler_T = ReduceLROnPlateau(optimizer_adam_T , factor = 0.5 , min_lr = 1e-5 , verbose=False )
    scheduler_psi = ReduceLROnPlateau(optimizer_adam_psi , factor = 0.5 , min_lr = 1e-5 , verbose=False )
    scheduler_omega = ReduceLROnPlateau(optimizer_adam_omega , factor = 0.5 , min_lr = 1e-5 , verbose=False )
    ...
    for epoch in range(epochs_adam):
        ...
        pde_loss = residula_loss(PINN_psi , PINN_omega  , PINN_T , X_c_test , Y_c_test )
        bc_loss = boundary_condition_loss(PINN_psi , PINN_omega  , PINN_T ,X_boundary_test[: , 0].reshape(-1 , 1) , X_boundary_test[: ,1].reshape(-1 , 1) , Y_boundary_test)
        interior_loss = data_loss(PINN_psi , PINN_omega  , PINN_T , X_interior_test[: , 0].reshape(-1 , 1) , X_interior_test[: ,1].reshape(-1 , 1) , Y_interior_test)
        #noisy_loss = noisy_data_loss(PINN_psi , PINN_omega  , PINN_T , x_interior , y_interior , output_interior)
        noisy_loss = residula_loss(PINN_psi , PINN_omega  , PINN_T , X_c_noisy , Y_c_noisy )
        test_loss = (pde_loss * lambda_pde) + (bc_loss * lambda_bc) + (interior_loss * lambda_interior) + (noisy_loss * w_noise)
        loss_val_history.append(test_loss.item())

        current_LR = optimizer_adam_psi.param_groups[0]['lr']
        # Print losses
        if epoch % 500 == 0:
            print(f"Epoch [{epoch+1}/{epochs_adam}], Train Loss: {loss.item():.4f}, Test Loss: {test_loss.item():.4f} // LearningRate psi :{current_LR} ")
            print(f"Loss PDE: {pde_loss.item():.6f}  |   loss Data: {interior_loss.item():.4f}  |   BC loss: {bc_loss.item():.4f}")
            print(f"======================================================================")
