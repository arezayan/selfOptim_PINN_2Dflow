...
def train_pinn(PINN_psi, PINN_omega , PINN_T , filename_data,
    ...
    y_inlet = normal_inputs(torch.tensor(np.linspace(0 , 5 , 122) , dtype = torch.float32).view(-1 , 1))
    x_inlet = normal_inputs(torch.tensor(np.zeros_like(y_inlet) , dtype = torch.float32).view(-1 , 1))
    T_boundary_explicit = normal_inputs(torch.tensor((300 * np.ones_like(y_inlet)) , dtype = torch.float32).view(-1 , 1))
    ...
    for epoch in range(epochs_adam):
        ...
        with torch.no_grad():
            PINN_T.eval()
            #Enforce explictly Inlet boundary
            PINN_T(torch.cat((x_inlet , y_inlet) , dim = 1)).copy_(T_boundary_explicit)
            PINN_T.train()
        ...
        # Print losses
        if epoch % 500 == 0:
            print(f"Epoch [{epoch+1}/{epochs_adam}] || Train Loss: {loss.item():.4f} || LearningRate psi :{current_LR} ")
            print(f"Loss PDE: {pde_loss.item():.6f}  |   loss Data: {interior_loss.item():.4f}  ")
            print(f"======================================================================")
